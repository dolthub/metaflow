...
class DoltMLDemoFlow(FlowSpec):
    testset_version = Parameter('testset-version',  help="Specifc the tag for the input version", required=True)
    @step
    def start(self):
        # Start by getting original dataset
        with DoltDT(run=self, doltdb_path='iris-test', branch=self.testset_version) as dolt:
